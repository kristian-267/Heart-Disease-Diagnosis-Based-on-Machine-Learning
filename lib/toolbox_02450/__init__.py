...
def rlr_validate(X,y,lambdas,cvf=10):
    ...
    train_err_nofeatures = np.empty(cvf)
    test_err_nofeatures = np.empty(cvf)
    f = 0
    y = y.squeeze()
    ...
    train_r2 = (train_err_nofeatures.sum() - train_error[:, np.where(lambdas==opt_lambda)].sum()) / train_err_nofeatures.sum()
    test_r2 = (test_err_nofeatures.sum() - test_error[:, np.where(lambdas==opt_lambda)].sum()) / test_err_nofeatures.sum()
    
    return opt_val_err, opt_lambda, mean_w_vs_lambda, train_err_vs_lambda, test_err_vs_lambda, train_r2, test_r2
...
def visualize_decision_boundary(predict, 
    ...
    # function predict()
    rd = np.reshape(grid, (2, -1)).T
    rN = rd.shape[0]
    rdata = np.zeros((rN, 28))
    rdata[:, i:i+2] = rd
    grid_predictions = predict(rdata)
    ...
    if show_legend:
        plt.legend([str(class_names[i])+' '+e for i in range(C) for e in ['train','test']],
